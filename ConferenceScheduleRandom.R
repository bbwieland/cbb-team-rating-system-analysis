...
# Generate Simulated Non-Con Schedule ---------------------------------------------
...
build_matchups <- function(teams1, teams2, offset) {
  ...
  if (length(id1) != length(id2)) {
    id1 <- sample(id1, length(id2))
  }
  
  if (offset >= length(id1) & offset >= length(id2)) {
...
# Build Game Weeks
noncon_games_to_play =  1:10
noncon_games <- map_dfr(.x = noncon_games_to_play, .f = ~ build_matchups(teamIDs.1H, teamIDs.2H, offset = .x))
...
build_conf_schedule_random <- function(conference, n_games = 11) {

  conference_ids <- team_ratings %>%
    filter(conf_random == conference) %>%
    pull(TeamID)
...
conf_games <- map_dfr(.x = unique(team_ratings$conf_random),
                      .f = ~ build_conf_schedule_random(.x))

# output: conf_games

# Combine Full Schedule ---------------------------------------------------

games <- rbind(noncon_games, conf_games)
...
ratings_final <- ratings_merge %>%
  mutate(schedule_type = "Random Conferences")

write_csv(ratings_final, "data/ConfRandom.csv")